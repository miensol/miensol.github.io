...
You cannot change the endpoint address of RDS database. 
It is something generated by AWS which you have no influence over.

As planned we want to update the DNS entry when migrating to VPC based RDS instance.
For this two work we need a custom DNS entry. One that we can control.

## Private hosted zone

Often, when deploying services inside a VPC it makes sense to use a private DNS hosted zone.
Such approach allows us to define e.g. `internal` DNS space which we can freely use.
...
This way once we change the record the application will establish connections to new database running
inside VPC.
...
The database host name that we set should now look like `database.internal`. Please note that we are 
...
The main issue is that the DNS update will not immediately cause all services to connect to new database.
In many cases this is fine. 
Moreover, we need to make sure that our database clients do not cache DNS entries for too long.

If you need the connections to be moved to new database instance in a more controllable fashion it is still possible.
A deployment with an update of the database configuration of services might be enough. 
If such a deployment is not an option we can introduce a network proxy e.g. using network load balancer, ProxySQL or HAProxy.   
